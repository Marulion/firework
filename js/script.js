...
// 高性能デバイスの検出。これは変動する目標となります。
const IS_HIGH_END_DEVICE = (() => {
	const hwConcurrency = navigator.hardwareConcurrency;
	if (!hwConcurrency) {
		return false;
	}
	//大画面はフルサイズのコンピューターで表示され、現代のコンピューターは通常ハイパースレッディングを搭載しています。
	//そのため、4コアのデスクトップPCは8コアを持ちます。そこでより高い最小閾値を設定します。
	const minCount = window.innerWidth <= 1024 ? 4 : 8;
	return hwConcurrency >= minCount;
})();
//不合理な画面サイズでキャンバスが大きくなりすぎるのを防ぎます。
// 8K - 必要に応じて制限可能
const MAX_WIDTH = 7680;
const MAX_HEIGHT = 4320;
const GRAVITY = 0.9; //ピクセル/秒単位の加速度
let simSpeed = 1;
...
//すべての品質グローバル変数は上書きされ、"configDidUpdate"を通じて更新されます。
let quality = 1;
...
//特殊な不可視色（レンダリングされないため、カラーマップには含まれません）
...
//フルスクリーンヘルパー、Fscreenをプレフィックスとして使用。
function fullscreenEnabled() {
	return fscreen.fullscreenEnabled;
}

//フルスクリーン状態はストレージと同期されることに注意してください。ストレージがソースとなります
//アプリケーションがフルスクリーンモードかどうかを判断します。
function isFullscreen() {
	return !!fscreen.fullscreenElement;
}

// フルスクリーンモードの切り替えを試みます。
...
// フルスクリーンの変更をストレージと同期します。イベントリスナーが必要です。
// ユーザーがブラウザで直接フルスクリーンを切り替えることができ、それに反応する必要があるためです。
...
const store = {
	...
	state: {
		...
		//<select>の設定値は文字列である必要があります。
		//レンダリング時に手動で文字列に変換し、変更時に解析する必要があります。
		config: {
			quality: String(IS_HIGH_END_DEVICE ? QUALITY_HIGH : QUALITY_NORMAL), // パフォーマンスのため、`configDidUpdate`でグローバル変数`quality`にミラーリングされます
			shell: "Random",
			size: IS_DESKTOP
				? "3" // デスクトップのデフォルト
				: IS_HEADER
...
const helpContent = {
	shellType: {
		header: "花火の種類",
		body: "打ち上げる花火の種類です。「ランダム（Random）」を選ぶと、とても良い体験ができます！",
	},
	shellSize: {
		header: "花火の大きさ",
		body: "花火が大きいほど開花範囲も大きくなりますが、デバイスに必要な性能も増加します。大きな花火はデバイスの動作が遅くなる可能性があります。",
	},
	quality: {
		header: "画質",
		body: "アニメーションがスムーズに動作しない場合は、画質を下げてみてください。画質が高いほど、花火の開花後の火花の数が多くなりますが、高画質はデバイスの動作が遅くなる可能性があります。",
	},
	skyLighting: {
		header: "空の明るさ",
		body: "花火が爆発すると、背景が明るくなります。画面が明るすぎる場合は、「暗い」または「なし」に変更できます。",
	},
	scaleFactor: {
		header: "拡大縮小",
		body: "花火との距離を近くまたは遠くにします。大きな花火の場合は、特にスマートフォンやタブレットで、より小さい拡大値を選択できます。",
	},
	wordShell: {
		header: "文字花火",
		body: "オンにすると、文字の形の花火が表示されます。例：明けましておめでとう、幸せになりますように など",
	},
	autoLaunch: {
		header: "自動打ち上げ",
		body: "オンにすると、デバイスの画面の前で花火を楽しむことができます。オフにすると、画面をタップして花火を打ち上げる必要があります。",
	},
	finaleMode: {
		header: "フィナーレモード",
		body: "同時により多くの花火を自動的に打ち上げることができます（「自動打ち上げ」をオンにする必要があります）。",
	},
	hideControls: {
		header: "コントロールを隠す",
		body: "画面上部のボタンを隠します。スクリーンショットを撮りたい場合や、シームレスな体験が必要な場合は、ボタンを隠すことができます。ボタンを隠した後でも、右上隅から設定を開くことができます。",
	},
	fullscreen: {
		header: "全画面表示",
		body: "全画面モードに切り替えます",
...
function init() {
	...
	setOptionsForSelect(appNodes.quality, [
		...
		{ label: "標準", value: QUALITY_NORMAL },
		{ label: "高", value: QUALITY_HIGH },
	]);

	setOptionsForSelect(appNodes.skyLighting, [
		{ label: "なし", value: SKY_LIGHT_NONE },
		{ label: "暗い", value: SKY_LIGHT_DIM },
		{ label: "標準", value: SKY_LIGHT_NORMAL },
	]);
...
if (IS_HEADER) {
	...
	setTimeout(() => {
		...
		Promise.all(promises).then(init, (reason) => {
			console.log("リソースファイルの読み込みに失敗しました");
			init();
...
function addCustomWord() {
    const inputWord = document.getElementById('userInput').value.trim();
    if (inputWord) {
        wordDotsMap[inputWord] = MyMath.literalLattice(inputWord, 3, "Gabriola,華文琥珀", "90px");
        randomWords.push(inputWord);
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const locations = [
		...
		{ name: "福冈", x: 25, y: 140, image: "./images/fukuoka.jpg" },
        { name: "大阪", x: 75, y: 155, image: "./images/osaka.webp" },
        { name: "神户", x: 65, y: 135, image: "./images/kobe.jpg" },
        { name: "京都", x: 85, y: 130, image: "./images/kyoto.jpg" },
        { name: "横滨", x: 120, y: 155, image: "./images/yokohama.jpg" },
        { name: "东京", x: 135, y: 120, image: "./images/tokyo.jpg" },
        { name: "仙台", x: 140, y: 90, image: "./images/sentai.jpg" },
        { name: "札幌", x: 145, y: 20, image: "./images/saboro.jpg" },
    ...
    locations.forEach(location => {
        ...
        marker.style.cssText = `
            ...
            background: pink;
            clip-path: path('M10,0 L13.09,6.58 L20,7.64 L15,12.76 L16.18,20 L10,16.58 L3.82,20 L5,12.76 L0,7.64 L6.91,6.58 Z');
        ...
        marker.addEventListener("mouseover", () => {
            ...
            marker.style.background = "#ff69b4";
        });
        
        marker.addEventListener("mouseout", () => {
            marker.querySelector(".tooltip").style.opacity = "0";
    ...
    document.querySelector(".stage-container").appendChild(miniMap);
});
